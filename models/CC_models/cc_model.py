...
class CepstralCoeffsEncoder(nn.Module):
    ...
    def encode(self, x):
        ...
        # print(x.sum()) #tensor(2.1695)
        # torchaudio.save(f'audio.wav', x, 44100)
        # mel_spec = librosa.feature.melspectrogram(x.cpu().numpy(), sr = 44100, n_fft = self.l_grain, hop_length=self.hop_size, n_mels=256)
        # print("Mel Sum: ", mel_spec.sum())
        # recon_audio = librosa.feature.inverse.mel_to_audio(mel_spec, sr = 44100, n_fft = self.l_grain, hop_length=self.hop_size)
        # print("Recon Audio Sum: ", recon_audio.sum())
        # torchaudio.save(f'recon_audio.wav', torch.from_numpy(recon_audio), 44100)
        ...
        # transform = torchaudio.transforms.GriffinLim(n_fft = 1024, hop_length = self.hop_size, power=1)
        # recon_audio = transform(inv_cepstral_coeff)
        ...
        # grain_db = 20*np.log10(np.abs(test_stft))
        # cepstral_coeff = fft.dct(grain_db)
        grain_db = 20*torch.log10(torch.abs(stft))
        cepstral_coeff = dct.dct(grain_db)

        # Take the first 128 cepstral coefficients - removing this for this
        ...
        # cepstral_coeff = cepstral_coeff.permute(0,2,1)
        # print("Cepstral Coeff shape: ", cepstral_coeff.shape)
...
class CepstralCoeffsDecoder(nn.Module):
    ...
    def decode(self, z, n_grains=None, ola_windows=None, ola_folder=None, ola_divisor=None):
        ...
        # audio_sum = torch.from_numpy(librosa.griffinlim(h.cpu().detach().numpy(), n_fft = 1024, hop_length = self.hop_size))
