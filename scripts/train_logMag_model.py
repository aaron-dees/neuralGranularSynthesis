...
from models.spectrogram_models.log_mag_model import RISpecVAE_v1, RISpecVAE_v2, RISpecVAE_v3
    ...
    if TRAIN:
        ...
        for epoch in range(start_epoch, EPOCHS):
            ...
            for data in train_dataloader:
                ...
                mag_spec = torch.abs(stft_audio).permute(0,2,1)
                mag_spec = mag_spec.reshape(stft_audio.shape[0]*n_grains, (int((l_grain//2)+1)))
                inv_cep_coeffs = inv_cep_coeffs.reshape(inv_cep_coeffs.shape[0]*n_grains, (int((l_grain//2)+1)))

                # transform = torchaudio.transforms.GriffinLim(n_fft=l_grain, hop_length=hop_size, power=1)
                # mag_spec = mag_spec.reshape(-1, n_grains, (int((l_grain//2)+1)))
                # recon_audio_test = transform(mag_spec.permute(0,2,1))
                # torchaudio.save(f'{RECONSTRUCTION_SAVE_DIR}/test_2.wav', recon_audio_test.cpu(), SAMPLE_RATE)

                compressionFactor=1
                mag_spec = torch.sigmoid(compressionFactor*mag_spec)
                ...
                x_hat = x_hat.reshape(-1, n_grains, (int((l_grain//2)+1)))
