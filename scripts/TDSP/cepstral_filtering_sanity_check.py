...
# plt.plot(torch.fft.rfft(noise)[7])
# plt.plot(inv_cepstral_coeff[7])
# plt.plot(noise[7])
# plt.savefig("white_noise_time.png")

print("Signal Noise Max (freq): ", sig_DivSpecShape.abs().max())
print("White Noise Max (freq): ", torch.fft.rfft(noise).abs().max())
print("Signal Noise Max (time): ", sig_noise.max())
print("White Noise Max (time): ", noise.max())
print("Signal Noise Min (time): ", sig_noise.min())
